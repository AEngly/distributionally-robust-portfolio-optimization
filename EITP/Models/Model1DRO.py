...
class IndexTracker:
    ...
    # Method 3: Run model
    def solve(self, epsCollection=np.linspace(10**(-8), 10**(-1), 100)):
        ...
        for k in range(K):
            ...
            #M.constraint('infinityNorm12{}'.format(k), Expr.sub(-a_k2[k], _lambda), Domain.lessThan(0.0));
            M.constraint('infinityNorm2{}'.format(k), Expr.sub(a_k1[k], Expr.mul(mOnes, _lambda)), Domain.lessThan(0.0));
            #M.constraint('infinityNorm22{}'.format(k), Expr.sub(a_k2[k], _lambda), Domain.lessThan(0.0));
        ...
        # Solve optimization
        with tqdm(total=len(epsCollection)) as pbar:
