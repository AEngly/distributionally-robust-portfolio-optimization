...
startEps = -9
endEps = 1
epsCollection = np.concatenate(([0], 10**np.linspace(startEps, endEps, 50)), axis=0)
...
epsOpt = np.zeros((nTrainingSizes, len(rebalanceIndices)))

# Allocate space for portfolio values
totalDays = allDates.shape[0]
trajectories = np.zeros((nTrainingSizes, totalDays,4))

# Allocate memory for statistics
nModels = 2
totalMetrics = 21
OoS_statistics = np.zeros((nModels, nTrainingSizes, len(rebalanceIndices), totalMetrics))
IS_statistics = np.zeros((nModels, nTrainingSizes, len(rebalanceIndices), totalMetrics))

# Save number of constituents
nAssetsRebalance = np.zeros(len(rebalanceIndices[:-1]))
...
with tqdm(total=len(rebalanceIndices[:-1])*nTrainingSizes, disable=False) as pbar:
    for TS, trainingSize in enumerate(trainingSizes):
        ...
        for k,idx in enumerate(rebalanceIndices[:-1]):
            ...
            # Save number of constituents
            nAssetsRebalance[k] = assetsReturns.shape[1]
            ...
            epsOpt[TS,k] = epsCollection[indexEpsOpt]
            ...
            results_SAA_DRO = MODEL.solve(epsCollection=np.array([0, epsOpt[TS,k]]), rhoCollection=rhoCollection,
                                    betaCollection=betaCollection, progressBar=False);
            ...
            for j, key in enumerate(metricsRecordings_SAA_IS):
                IS_statistics[0,TS,k,j] = metricsRecordings_SAA_IS[key]
                IS_statistics[1,TS,k,j] = metricsRecordings_DRO_IS[key]
                OoS_statistics[0,TS,k,j] = metricsRecordings_SAA_OoS[key]
                OoS_statistics[1,TS,k,j] = metricsRecordings_DRO_OoS[key]
            ...
            if Tdx == 0:
                ...
                trajectories[TS,fromIndex:toIndex,0] = np.copy(index_OoS.flatten()/100)
                trajectories[TS,fromIndex:toIndex,1] = np.copy(enhancedIndex_OoS.flatten()/100)
                trajectories[TS,fromIndex:toIndex,2] = np.copy(portfolioSAA_OoS.flatten()/100)
                trajectories[TS,fromIndex:toIndex,3] = np.copy(portfolioDRO_OoS.flatten()/100)

                # Overwrite with percentage changes instead
                trajectories[TS,fromIndex+1:toIndex,0] = np.diff(trajectories[TS,fromIndex:toIndex,0])/trajectories[TS,fromIndex:toIndex-1,0] + 1
                trajectories[TS,fromIndex+1:toIndex,1] = np.diff(trajectories[TS,fromIndex:toIndex,1])/trajectories[TS,fromIndex:toIndex-1,1] + 1
                trajectories[TS,fromIndex+1:toIndex,2] = np.diff(trajectories[TS,fromIndex:toIndex,2])/trajectories[TS,fromIndex:toIndex-1,2] + 1
                trajectories[TS,fromIndex+1:toIndex,3] = np.diff(trajectories[TS,fromIndex:toIndex,3])/trajectories[TS,fromIndex:toIndex-1,3] + 1
                ...
                # Overwrite with percentage changes instead
                trajectories[TS,fromIndex:toIndex,0] = np.diff(indexNormalized)/indexNormalized[:-1] + 1
                trajectories[TS,fromIndex:toIndex,1] = np.diff(enhancedIndexNormalized)/enhancedIndexNormalized[:-1] + 1
                trajectories[TS,fromIndex:toIndex,2] = np.diff(portfolioSAANormalized)/portfolioSAANormalized[:-1] + 1
                trajectories[TS,fromIndex:toIndex,3] = np.diff(portfolioDRONormalized)/portfolioDRONormalized[:-1] + 1
...
# Save constituents on each rebalance date
# dataFrameConstituents = pd.DataFrame(nAssetsRebalance, columns=["Constituents"])
# dataFrameConstituents.to_csv("./Results/Backtest/ITM/Constituents.csv", index=False)

# Save Wasserstein parameters
dataFrameTrajectoryDates = pd.DataFrame(allDates.values, columns=["Dates"])
dataFrameTrajectoryDates.to_csv("./Results/Backtest/ITM/TrajectoryDates.csv", index=False)

# Save Wasserstein parameters
dataFrameRebalanceDates = pd.DataFrame(rebalanceDates, columns=["Dates"])
dataFrameRebalanceDates.to_csv("./Results/Backtest/ITM/RebalanceDates.csv", index=False)

# Save core results to file with id

# Create logging string
line0 = "fileName: " + "Backtest_ITM_Trajectories" + "\n"
...
# Save results
write_parameters_to_file(trajectories.reshape(-1), parameters, file_name="Backtest_ITM_Trajectories", folder_path="./Results/Backtest/ITM/", expId=1)

# Create logging string
line0 = "fileName: " + "Backtest_ITM_Wasserstein" + "\n"
line1 = "alpha: {}".format(excessReturnAnually) + "\n"
line2 = "nEps: " + str(len(epsCollection)) + "\n"
line3 = "nBetas: " + str(len(betaCollection)) + "\n"
line4 = "nRhos: " + str(len(rhoCollection)) + "\n"
...
# Save results
write_parameters_to_file(epsOpt.reshape(-1), parameters, file_name="Backtest_ITM_Wasserstein", folder_path="./Results/Backtest/ITM/", expId=1)
...
# Create logging string
line0 = "fileName: " + "Backtest_ITM_epsOpt" + "\n"
line1 = "alpha: {}".format(excessReturnAnually) + "\n"
line2 = "nEps: " + str(len(epsCollection)) + "\n"
line3 = "nBetas: " + str(len(betaCollection)) + "\n"
line4 = "nRhos: " + str(len(rhoCollection)) + "\n"
...
# Save results
write_parameters_to_file(epsOpt, parameters, file_name="Backtest_ITM_epsOpt", folder_path="./Results/Backtest/ITM/", expId=1)

# Save statistics

# Create logging string
line0 = "fileName: " + "Backtest_ITM_IS_statistics" + "\n"
line1 = "alpha: {}".format(excessReturnAnually) + "\n"
line2 = "nEps: " + str(len(epsCollection)) + "\n"
line3 = "nBetas: " + str(len(betaCollection)) + "\n"
line4 = "nRhos: " + str(len(rhoCollection)) + "\n"
...
# Save results
write_parameters_to_file(IS_statistics.reshape(-1), parameters, file_name="Backtest_ITM_IS_statistics", folder_path="./Results/Backtest/ITM/", expId=1)

# Create logging string
line0 = "fileName: " + "Backtest_ITM_OoS_statistics" + "\n"
line1 = "alpha: {}".format(excessReturnAnually) + "\n"
line2 = "nEps: " + str(len(epsCollection)) + "\n"
line3 = "nBetas: " + str(len(betaCollection)) + "\n"
line4 = "nRhos: " + str(len(rhoCollection)) + "\n"
...
# Save results
write_parameters_to_file(OoS_statistics.reshape(-1), parameters, file_name="Backtest_ITM_OoS_statistics", folder_path="./Results/Backtest/ITM/", expId=1)